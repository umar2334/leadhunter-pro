...
export default function LeadDetailPage() {
  ...
  if (!lead) return (
    <div className="min-h-screen bg-[#0a0a12] flex items-center justify-center text-slate-500">Lead not found</div>
  );
  ...
  const scoreColor = !lead.analysis_score ? '#64748b'
    ...
    : lead.analysis_score < 65 ? '#f97316' : '#4ade80';
    ...
    <>
      ...
      <style>{`
        @media print {
          ...
          .print-sub { color: #444 !important; }
        }
      `}</style>

      <div className="min-h-screen bg-[#0a0a12]">
        {/* Nav */}
        <nav className="border-b border-[#2d2d3d] px-6 py-4 flex items-center justify-between no-print">
          ...
          <div className="flex items-center gap-3">
            <button onClick={printReport}
              className="flex items-center gap-1.5 text-xs bg-[#1a1a2e] border border-[#2d2d3d] hover:border-indigo-500 text-slate-400 hover:text-white px-3 py-1.5 rounded-lg transition-colors">
        ...
        <main className="max-w-4xl mx-auto px-6 py-8">
          {/* Business Header */}
          <div className="bg-[#12121e] border border-[#2d2d3d] rounded-2xl p-6 mb-5 print-card">
            <div className="flex flex-wrap items-start justify-between gap-4">
              <div>
                <h1 className="text-2xl font-bold text-white print-text">{lead.name}</h1>
                {lead.category && <span className="text-sm text-slate-500 mt-1 block">{lead.category}</span>}

                <div className="flex flex-wrap gap-4 mt-4">
                  {lead.phone && (
                    <div className="flex items-center gap-2">
                      <a href={`tel:${lead.phone}`} className="flex items-center gap-2 text-sm text-slate-400 hover:text-indigo-400">
                        <Phone className="w-4 h-4" /> {lead.phone}
                      </a>
                      <button onClick={() => copyText(lead.phone!, 'phone')} className="text-slate-600 hover:text-slate-300 no-print">
                        <Copy className="w-3 h-3" />
                      </button>
                      {copied === 'phone' && <span className="text-xs text-green-400">Copied!</span>}
                    </div>
                  )}
                  {lead.email && (
                    <div className="flex items-center gap-2">
                      <a href={`mailto:${lead.email}`} className="flex items-center gap-2 text-sm text-indigo-400 hover:text-indigo-300">
                        <Mail className="w-4 h-4" /> {lead.email}
                      </a>
                      <button onClick={() => copyText(lead.email!, 'email')} className="text-slate-600 hover:text-slate-300 no-print">
                        <Copy className="w-3 h-3" />
                      </button>
                      {copied === 'email' && <span className="text-xs text-green-400">Copied!</span>}
                    </div>
                  )}
                  {lead.address && (
              ...
              <div className="flex flex-col items-end gap-3">
                <span className={`text-sm font-semibold px-3 py-1.5 rounded-lg border ${oppColor.bg} ${oppColor.text} ${oppColor.border}`}>
                  {lead.opportunity_type === 'no_website' ? '🔥 No Website'
                    : lead.opportunity_type === 'weak_website' ? '⚠️ Weak Website' : '✅ Has Website'}
                </span>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5 mb-5">
            {/* Website Analysis */}
            <div className="bg-[#12121e] border border-[#2d2d3d] rounded-2xl p-5 print-card">
              <div className="flex items-center justify-between mb-4">
                <h2 className="text-sm font-semibold text-white print-text">Website Analysis</h2>
                {lead.website && (
                  <button onClick={reanalyze} disabled={reanalyzing}
                    className="text-xs text-slate-500 hover:text-slate-300 flex items-center gap-1 no-print">
                    <RefreshCw className={`w-3 h-3 ${reanalyzing ? 'animate-spin' : ''}`} /> Re-analyze
                  </button>
                )}
              </div>
                    ...
                    <div className="space-y-1.5 mb-3">
                      {lead.analysis_issues.map((issue, i) => (
                        <div key={i} className="flex items-start gap-2 text-xs text-slate-400">
                          <AlertCircle className="w-3.5 h-3.5 text-orange-500 flex-shrink-0 mt-0.5" />{issue}
                        </div>
                      ))}
                    </div>
                  ) : null}
            ...
            <div className="flex flex-col gap-4">
              <div className="bg-[#12121e] border border-[#2d2d3d] rounded-2xl p-5 print-card no-print">
                <h2 className="text-sm font-semibold text-white mb-3">Outreach</h2>
                <p className="text-xs text-slate-500 mb-4">
                  AI-personalized email, WhatsApp & call script based on this lead's specific situation.
                </p>
              ...
              <div className="bg-[#12121e] border border-[#2d2d3d] rounded-2xl p-5 flex-1 print-card">
                <h2 className="text-sm font-semibold text-white mb-3">Notes</h2>
                <textarea
                  className="w-full bg-[#0a0a12] border border-[#2d2d3d] rounded-lg text-sm text-slate-300 p-3 resize-none focus:outline-none focus:border-indigo-500 leading-relaxed no-print"
                  rows={4} placeholder="Add notes about this lead..."
                  value={notes} onChange={(e) => setNotes(e.target.value)} />
                {notes && <p className="text-xs text-slate-400 hidden print:block">{notes}</p>}
                <button onClick={saveNotes} disabled={savingNotes || notes === lead.notes}
                  className="mt-2 text-xs bg-[#1a1a2e] border border-[#2d2d3d] hover:border-indigo-600 text-slate-400 hover:text-white px-3 py-1.5 rounded-lg transition-colors disabled:opacity-40 no-print">
          ...
          {lead.maps_url && (
            <a href={lead.maps_url} target="_blank" rel="noopener noreferrer"
              className="inline-flex items-center gap-2 text-xs text-slate-600 hover:text-slate-400 transition-colors no-print">
